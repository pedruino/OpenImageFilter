...
Bitmap::Bitmap(const Bitmap &self)
{
    ...
    std::memcpy(this->_header, self._header, sizeof(self._header));
    ...
    this->_pixelData = new unsigned char[this->_pixelArrayImageSize];
    std::memcpy(this->_pixelData, self._pixelData, this->_pixelArrayImageSize);
}
...
void Bitmap::Save(const char* image_path)
{
    int volume = this->_height * this->_stride;

    FILE* w = fopen(image_path, "wb");
    fwrite(this->_header, sizeof(unsigned char), 54, w);
    fwrite(this->_pixelData, sizeof(unsigned char), volume, w);
    fclose(w);
}

void Bitmap::SetPixelData(unsigned char *data)
{
    memcpy(this->_pixelData, data, this->_pixelArrayImageSize);
}
